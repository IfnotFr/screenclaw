  ...
  <div class="h-full flex flex-col">
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-3">
        <RouterLink to="/missions" class="text-zinc-500 hover:text-zinc-100 text-sm">← Missions</RouterLink>
        <span class="text-zinc-700">/</span>
        <h1 class="text-lg font-semibold">{{ id }}</h1>
      </div>
      <div class="flex gap-2">
        <RouterLink to="/missions" class="text-sm text-zinc-400 hover:text-zinc-100 px-3 py-1.5">Cancel</RouterLink>
...
onMounted(async () => {
  const data = await fetch(`/api/missions/${id}`).then(r => r.json());
